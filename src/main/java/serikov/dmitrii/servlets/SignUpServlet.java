...
public class SignUpServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			UserProfile userProfile = (UserProfile) Utils.fromJson(SignUpServlet.class.getName(), request,
					UserProfile.class);
			...
			if (DBUtils.connect()) {
				...
				// ResponseJsonObject responseJsonObject = new
				// ResponseJsonObject();
				...
				if (DBUtils.checkUserExistance(username)) {
					...
					if (DBUtils.createUser(username, password, avatar)) {
						...
						// get session object and set session relative
						// attributes
						...
						response.sendError(HttpServletResponse.SC_CONFLICT,
								"ServerError: User cannot be created in database");
